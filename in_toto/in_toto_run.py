...
""".format(
    ...
    named_args.add_argument(*PKCS8_ARGS, **PKCS8_KWARGS)
...
def main():
    ...
    # Only use one of legacy key, gpg, pkcs8 key, or no key at all.
    if sum([bool(args.key), bool(args.gpg), bool(args.pkcs8)]) > 1:
        parser.print_usage()
        parser.error(
            "Specify only one of '--key <key path>', "
            "--gpg [<keyid>]' or --pkcs8 <path>"
        )

...
def parse_packet_header(data, expected_type=None):
  ...
  # New Format Packet Lengths if Bit 6 of 1st octet is on, Old Format Packet
  # Lengths otherwise
  if data[0] & 0x40:
    # In new format packet lengths the packet type is encoded in Bits 5-0 of
    # the 1st octet of the packet (see RFC 4880 4.3. for the list of types).
    ...
    if length_type == 0:
      ...
      raise in_toto.gpg.exceptions.PacketParsingError("Old format packets of"
          " indeterminate length are not supported")

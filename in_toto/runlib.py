...
from in_toto.models.link import (FILENAME_FORMAT_UNFINISHED,
    FILENAME_FORMAT_SHORT, FILENAME_FORMAT)
...
def in_toto_mock(name, link_cmd_args):
  ...
  link = in_toto_run(name, ["."], ["."], link_cmd_args, key=False,
      record_streams=True)
  link_metadata = Metadata(signed=link)
...
def in_toto_run(name, material_list, product_list,
  ...
  <Returns>
    Newly created Metadata object containing a link
  """
...
def in_toto_record_start(step_name, key, material_list):
  ...
  <Purpose>
    ...
    with passed key and stores it to disk with FILENAME_FORMAT_UNFINISHED.
  ...
  <Side Effects>
    Creates a file FILENAME_FORMAT_UNFINISHED

  <Returns>
    None.

  """
  unfinished_fn = FILENAME_FORMAT_UNFINISHED.format(step_name=step_name,
      keyid=key["keyid"])
  ...
  link_metadata.dump(filename=unfinished_fn)
...
def in_toto_record_stop(step_name, key, product_list):
  ...
  <Side Effects>
    ...
    Removes unfinished link file FILENAME_FORMAT_UNFINISHED from disk
  ...
  # Expects an a file with name FILENAME_FORMAT_UNFINISHED in the current dir
